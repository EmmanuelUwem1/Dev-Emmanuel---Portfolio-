...
function Portfolio() {
  ...
  return (
    <section className="px-4 flex flex-col items-center justify-start md:px-20 py-20 md:pt-28 b-rounded-bottom overflow-hidden relative mesh-gradient">
      ...
      <div className="flex flex-row gap-6 justify-center items-center  w-full mt-28 flex-wrap md:flex-nowrap">
        {projects.map((project) => (
            <Link
                href={project.href}
            key={project.title}
            className="flex flex-col w-full sm:w-72 md:w-full  bg-slate-800 rounded-2xl h-72 overflow-hidden "
          >
